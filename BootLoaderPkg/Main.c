...
void CopyLoadSegments(Elf64_Ehdr* ehdr) {
  ...
  for (Elf64_Half i=0;i<ehdr->e_phnum;i++) {
    ...
    UINT64 segm_in_file = (UINT64)ehdr + phdr[i].p_offset; // セグメントの開始位置
    CopyMem((VOID*)phdr[i].p_vaddr, (VOID*)segm_in_file, phdr[i].p_filesz);

    // 例）.bssセクションが含まれるセグメント
    // memsz > p_fileszとなる場合がある。このような場合を考慮して0で初期化している。
...
EFI_STATUS EFIAPI UefiMain(
  ...
  status = OpenRootDir(image_handle, &root_dir); // root_dirにはアドレスを書き込みたい→ポインタのポインタになっている
  if (EFI_ERROR(status)) {
  ...
  if (EFI_ERROR(status)) {
    ...
    status = gBS->ExitBootServices(image_handle, memmap.map_key);
    if (EFI_ERROR(status)) {
      Print(L"cound not exit boot service: %r\n", status);
      Halt(); 
    }   
  }
