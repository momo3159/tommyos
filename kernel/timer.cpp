...
void InitializeLAPICTimer(std::deque<Message>& msg_queue) {
  ...
  StartLAPICTimer();
  acpi::WaitMiliseconds(100);
  const auto elapsed = LAPICTimerElapsed();
  StopLAPICTimer();

  lapic_timer_freq = static_cast<unsigned long>(elapsed) * 10; // 1000ms == 1s で 増えたLAPIC Timerのカウントの数 
  divide_config = 0b1011;
  lvt_timer = (0b010 << 16) | InterruptVector::kLAPICTimer;

  initial_count = lapic_timer_freq / kTimerFreq; // 1 / kTimerFreq 秒のカウント数
}

void StartLAPICTimer() {
  // Initial Count に値を書き込むことでタイマが開始
