...
void DrawTextCursor(bool visible) {
  ...
  const auto pos = Vector2D<int>{4 + 8 * current_index, 5};
  FillRectangle(*text_window->InnerWriter(), pos, {7, 15}, color);
}

void InputTextWindow(char c) {
  const auto max_chars = (text_window->InnerSize().x - 8) / 8 - 1;
  auto pos = []() { return Vector2D<int>{4 + 8*current_index, 6}; };
...
extern "C" void KernelMainNewStack(
  ...
  task_manager->NewTask()
    .InitContext(TaskIdle, 0xdeabbeef)
    .Wakeup();
  task_manager->NewTask()
