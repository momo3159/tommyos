...
namespace { // NOTE: 無名namespace. ファイルスコープ内でのみ有効な変数・関数を宣言
  ...
  Error ScanFunction(uint8_t bus, uint8_t device, uint8_t function) {
    ...
    if (class_code.Match(0x06u, 0x04u)) { // NOTE: PCI to PCI bridge
...
namespace pci{
  ...
  Error ScanAllBus() {
    ...
    auto header_type = ReadHeaderType(0, 0, 0); // ホストブリッジ
  ...
  void WriteConfReg(const Device& dev, uint8_t reg_addr, uint32_t value) {
    auto address = MakeAddress(dev.bus, dev.device, dev.function, reg_addr);
    WriteAddress(address);
    WriteData(value);
  }
} 