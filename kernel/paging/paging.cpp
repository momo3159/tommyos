...
namespace {
  ...
  alignas(kPageSize4K) std::array<std::array<uint64_t, 512>, kPageDirectoryCount> page_directory; // 8バイト * 512 = 4096B → 各page_directoryは4KiBで整列される
}

void SetupIdentityPageTable() {
  // UEFIによって恒等マッピングされている。そのため、プログラムから物理アドレスを指定できる
  pml4_table[0] = reinterpret_cast<uint64_t>(&pdp_table[0]) | 0x003; // ページがメモリ内にあり、かつ書き込みを許可。カーネルモードのみアクセスできる。
